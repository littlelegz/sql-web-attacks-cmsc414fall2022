...
Task 2: SQL Injection on UPDATE Statements
  ...
  format:
    ...
    3-n: One line for each profile parameter you changed, like so:
	 ...
	 Please note that both the parameter name and new value
	 should be wrapped in double quotes, so that we can clearly
	 see any spaces you might have added. Also, the parameter
	 name should be exactly as it appears in the HTML source code,
	 *not* what you see on the webpage. You will have to examine
	 the HTML or network traffic for these. (Hint: If your
	 parameters begin with capital letters, you have the wrong
	 parameter names.) The new value should be *exactly* what
	 you type into the corresponding field of the form, including
	 the injection.  If you don't do this, the automated grading
	 will mark this task as failing. *All* parameters that you
	 need to set must be included here for the automated grading
	 to succeed.

    n+1: A blank line 

    n+2 to end: The remaining lines should contain a short write
		up explaining the steps you used to create a working
		SQL injection attack that updates Bob's password
		to a new value.  This is especially important if
		you mis-format the parameter lines, since it's the
		most likely way we'll be able to figure out what
		you actually should have entered during manual
		regrading, if needed.
...
Task 3: Stealing Cookies from the Victim's machine 
  ...
  Note 2: Some popular software, like Android Studio, uses port 5555. If you're
  having issues with not being able to start the echo server, try changing 5555
  to 5556 (or some other number) everywhere you use it. Keep in mind that you'll
  need to make sure task3.txt is using 5555 when you're done, since that's how
  we're testing your submission.

  Note 3: If you have trouble building the echo server, you can
  also use ncat as an alternative. The nmap project's version is
  available at https://nmap.org/ncat/, but note that (on Windows)
  it will not be added to your system path. You either need to add
  it to the path, or run it from the nmap installation directory.
  The appropriate command to run is

    ncat --recv-only -l localhost 5555

  you might need to hit Ctrl-C to terminate ncat before the output appears.
  ...
  should be printed by the echo server. If you had to use a different port, don't
...
Task 4: Session Hijacking using the Stolen Cookies   
  ...
  program; it *will not* be passed on the command line. The first
  line of the input file contains the __elgg_ts token (absolute
  value), the second line contains the __elgg_token (absolute value)
...
Task 5: CSRF Attack using GET Request 
  ...
  sends Boby a URL (via an email or a posting in Elgg); Boby, curious about
  it, and clicks on the URL. Pretend that you are Alice, and think about how you can
  construct the contents of the web page, so as soon as Boby visits the web
  page, Alice is added to the friend list of Boby (assuming Boby has an active
  session with Elgg).
  ...
  successful as soon as Boby visits the web page, without even clicking
  ...
  localhost:<<port>>/task5.html in another tab, Alice should be added as a friend
  to Boby's Friend List.  
  ...
  reload/refresh button to reload the page, to see if Alice is added as a friend
  to Boby's account.
